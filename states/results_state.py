...
def handle_results(game, dt):
    ...
    if game.state == "arcade_results":
        ...
        final_text = "Player wins the Arcade session!" if winner == "PLAYER" else \
                     "AI wins the Arcade session!" if winner == "AI" else "It's a tie!"
        scores = [f"Player wins: {game.arcade.player_wins}", f"AI wins: {game.arcade.ai_wins}"]
    else:  # gp_results
        ...
        final_text = "Player wins the Grand Prix!" if winner == "PLAYER" else \
                     "AI wins the Grand Prix!" if winner == "AI" else "Tie!"
        scores = [
            f"Cup: {cup}",
            f"Player wins: {game.grand_prix.player_wins}",
            f"AI wins: {game.grand_prix.ai_wins}",
        ]
    ...
    for line in scores:
        ...
        y += 30
    screen.blit(FONT_SMALL.render(final_text, True, (245,245,200)),
                FONT_SMALL.render(final_text, True, (245,245,200)).get_rect(center=(cx, y+20)))

    back_btn = Button((cx-140, SCREEN_HEIGHT//2 + 100, 280, 50),
                      "Back to Main Menu", FONT_BTN,
