...
if __name__ == "__main__":
    ...
    checkpoint_cb = CustomCheckpointCallback(save_freq=500, save_path="./models/", name_prefix="ppo_race_quick_custom")
    eval_env_fn = DummyVecEnv([make_eval_env()])
    eval_env = VecFrameStack(eval_env_fn, n_stack=6)  # Align stacking
    eval_cb = CustomEvalCallback(eval_env, eval_freq=500, log_path="./logs/", n_eval_episodes=1)

    # Train with them (uncomment eval_cb if desired; quick run uses checkpoint only)
    model.learn(total_timesteps=4000, callback=[checkpoint_cb])#, eval_cb])
